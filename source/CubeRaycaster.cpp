...
//#define RUN_ON_CPU
#include <Common.hpp>
namespace kernels { struct Raycaster_Cube; }
using namespace cl::sycl;




const auto densityFunc = [](const float3& location)
{
		return 1;
};
...
const auto colorFunc = [](const int density)
{
	if (density > 0)
	{
		return float4(0.01, 0.01, 0.01, 1); // fehér
	}
	else
	{
		return float4(0, 0, 0, 1); // fekete	
	}
...

{
	...
	while (current_t < endT && !isSaturated)
	{
		...
		if (isInside(extent, location)) {
			...
			float4 color = colorFunc(densityFunc(location));
	...
	//finalColor.set_value(W, 1.0f);

	return finalColor;
};

CubeRaycaster::CubeRaycaster(std::size_t plat,
	std::size_t dev,
	cl_bitfield type,
	QWindow* parent)
...
#ifdef RUN_ON_CPU
void CubeRaycaster::updateSceneImpl() {
...
void CubeRaycaster::updateSceneImpl() {
	...
	compute_queue.submit([&](cl::sycl::handler& cgh)
	{
		...
		// scaleFOV?
		cgh.parallel_for<kernels::Raycaster_Cube>(range<2>{ new_lattice.get_range() },
